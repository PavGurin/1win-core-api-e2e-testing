...
export function getDateDaysAgo(daysAgo) {
  ...
  date.getDate() > 10 ? result.formatted = `${date.getDate()}/` : result.formatted = `0${date.getDate()}/`;
  date.getMonth() > 9 ? result.formatted += `${date.getMonth() + 1}` : result.formatted += `0${date.getMonth() + 1}`;
  return result;
}
