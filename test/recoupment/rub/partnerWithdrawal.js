...
describe('User with withdrawal from partner', () => {
  it('C2001948 (+) before 20 sept 19, one deposit from partner', async () => {
    ...
    const { data: withdrawalCreate } = await banking.checkWithdrawalPossible(10);
    // console.log(withdrawalCreate);
    expect(withdrawalCreate.result).toEqual(true);
  });
  ...
  it('C2001949 (-) before 20 sept 19, partner + card', async () => {
    ...
    const { data: withdrawalCreate } = await banking.checkWithdrawalPossible(10);
    // console.log(withdrawalCreate);
    expect(withdrawalCreate.result).toEqual(false);
  });
