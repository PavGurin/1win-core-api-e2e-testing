...
describe.skip('Create deposit for eth_usd - RUB @master', () => {
  ...
  it(' - (+) amount = 151 & wallet = (+7)phone', async () => {
    const { data } = await banking.depositCreateRequest(socket, '+79001234567', paymentType, currency, 151);
    // console.log(data);
  ...
  it(' - min amount & wallet = symbols', async () => {
    const { data } = await banking.depositCreateRequest(socket, '+79215598289', paymentType, currency, 150);
...
describe.skip('Create deposite for eth_usd invalid - RUB', () => {
  ...
  it(' - amount double < min amount', async () => {
    const { data } = await banking.depositCreateRequest(socket, '79215598386', paymentType, currency, 150.6);
    // console.log(data);
    checkErrMsg(data, 400, 'Неверная сумма');
  });

  it(' - amount < min amount', async () => {
    const { data } = await banking.depositCreateRequest(socket, '79215598486', paymentType, currency, 149);
    // console.log(data);
    checkErrMsg(data, 400, 'Неверная сумма');
  });
