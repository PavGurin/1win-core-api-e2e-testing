...
describe.skip('Create deposite for yamoney_ru - RUB @master', () => {
  ...
  it(' - (+) amount = 100.01 & wallet = symbols', async () => {
    const { data } = await banking.depositCreate(socket, '123 autotests', paymentType, currency, 100.1);
    // console.log(data);
    successDepositCreate(data, currency,
      paymentType, 100.1);
  });
  ...
  it('! - (+) amount = 100.01 & wallet = symbols', async () => {
    const { data } = await banking.depositCreate(socket, '123 autotests', paymentType, currency, 100.156);
    // console.log(data);
    successDepositCreate(data, currency,
      paymentType, 100.156);
  });
...
describe.skip('Create deposite for yamoney_ru invalid - RUB', () => {
  ...
  it(' - amount = 0', async () => {
    const { data } = await banking.depositCreate(socket, '', paymentType, currency, 0);
    // console.log(data);
    checkErrMsg(data, 400, 'Bad request, amount is invalid');
  });

  it(' - amount = null', async () => {
    const { data } = await banking.depositCreate(socket, '', paymentType, currency, null);
    // console.log(data);
    checkErrMsg(data, 400, 'Bad request, amount is required, no default value provided');
  });

  it(' - amount = empty', async () => {
    const { data } = await banking.depositCreate(socket, '', paymentType, currency, ' ');
    // console.log(data);
    checkErrMsg(data, 400, 'Bad request, amount is invalid');
  });

  it(' - amount = undefined', async () => {
    const { data } = await banking.depositCreate(socket, '', paymentType, currency, undefined);
    // console.log(data);
    checkErrMsg(data, 400, 'Bad request, amount is required, no default value provided');
  });

  it(' - amount = string', async () => {
    const { data } = await banking.depositCreate(socket, '', paymentType, currency, 'fjfj');
    // console.log(data);
    checkErrMsg(data, 400, 'Bad request, amount should have a type of number, but found string');
  });

  it(' - amount =string - number', async () => {
    const { data } = await banking.depositCreate(socket, '', paymentType, currency, '50');
    // console.log(data);
  ...
  it(' - wallet = long string', async () => {
    const { data } = await banking.depositCreate(socket, '1231231231231231453453345345342312312312312123123123123', paymentType, currency, 100);
    // console.log(data);
    checkErrMsg(data, 400, 'Неверный формат кошелька');
  });
  ...
  it(' - incorrect paymentType = yamoney_ru_test', async () => {
    const { data } = await banking.depositCreate(socket, '12312312', 'yamoney_ru_test', currency, 100);
    // console.log(data);
