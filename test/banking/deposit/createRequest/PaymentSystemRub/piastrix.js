...
describe.skip('Create deposite for piastrix_rub - RUB @master', () => {
  ...
  it(' - min amount & wallet = symbols', async () => {
    const { data } = await banking.depositCreateRequest(socket, '123234345456 etryrt', paymentType, currency, 1);
  ...
  it(' - > min amount & wallet = symbols', async () => {
    const { data } = await banking.depositCreateRequest(socket, '12№%:№%:45456etryrt', paymentType, currency, 2);
  ...
  it(' - max amount & wallet = numbers', async () => {
    const { data } = await banking.depositCreateRequest(socket, '09090909999', paymentType, currency, 100000);
    // console.log(data);
    successDepositCreate(data, currency,
      paymentType, 100000);
  });
...
describe.skip('Create deposite for piastrix_rub invalid - RUB', () => {
  ...
  it(' - wallet = null', async () => {
    const { data } = await banking.depositCreateRequest(socket, null, paymentType, currency, 1);
    // console.log(data);
    checkErrMsg(data, 400, 'Неверный формат кошелька');
  });

  it(' - wallet = long string', async () => {
    const { data } = await banking.depositCreateRequest(socket, '1231231231231231453453345345342312312312312123123123123', paymentType, currency, 1);
    // console.log(data);
    checkErrMsg(data, 400, 'Неверный формат кошелька');
  });
  ...
  it(' - incorrect paymentType = piastrix_rub_test', async () => {
    const { data } = await banking.depositCreateRequest(socket, '12312312312', 'piastrix_rub_test', currency, 1);
    // console.log(data);
