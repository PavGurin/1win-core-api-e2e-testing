...
describe.skip('Create deposite for megafon_rub - RUB @master', () => {
  ...
  it(' - max amount & wallet = numbers', async () => {
    const { data } = await banking.depositCreateRequest('+79001234567', paymentType, currency, 15000);
    // console.log(data);
    successDepositCreate(data, currency,
      paymentType, 15000);
  });
  ...
  it(' - < max amount & wallet = numbers', async () => {
    const { data } = await banking.depositCreateRequest('+79001234567', paymentType, currency, 14999);
    // console.log(data);
    successDepositCreate(data, currency,
      paymentType, 14999);
  });
...
describe.skip('Create deposite for megafon_rub invalid - RUB', () => {
  ...
  it(' - amount = 0', async () => {
    const { data } = await banking.depositCreateRequest('+79001234567', paymentType, currency, 0);
    // console.log(data);
    checkErrMsg(data, 400, 'Bad request, amount is invalid');
  });

  it(' - amount = null', async () => {
    const { data } = await banking.depositCreateRequest('+79001234567', paymentType, currency, null);
    // console.log(data);
    checkErrMsg(data, 400, 'Bad request, amount is required, no default value provided');
  });

  it(' - amount = empty', async () => {
    const { data } = await banking.depositCreateRequest('+79001234567', paymentType, currency, ' ');
    // console.log(data);
    checkErrMsg(data, 400, 'Bad request, amount is invalid');
  });

  it(' - amount = undefined', async () => {
    const { data } = await banking.depositCreateRequest('+79001234567', paymentType, currency, undefined);
    // console.log(data);
    checkErrMsg(data, 400, 'Bad request, amount is required, no default value provided');
  });

  it(' - amount = latanic', async () => {
    const { data } = await banking.depositCreateRequest('+79001234567', paymentType, currency, 'fjfj');
    // console.log(data);
  ...
  it(' - amount > max amount', async () => {
    const { data } = await banking.depositCreateRequest('+79001234567', paymentType, currency, 15001);
    // console.log(data);
    checkErrMsg(data, 400, 'Неверная сумма');
  });

  it(' - amount double > max amount', async () => {
    const { data } = await banking.depositCreateRequest('+79001234567', paymentType, currency, 15000.000001);
    // console.log(data);
    checkErrMsg(data, 400, 'Неверная сумма');
  });
  ...
  it(' wallet = long phone', async () => {
    const { data } = await banking.depositCreateRequest('+790012345670909', paymentType, currency, 100);
    // console.log(data);
    checkErrMsg(data, 400, 'Неверный формат кошелька');
  });
});
