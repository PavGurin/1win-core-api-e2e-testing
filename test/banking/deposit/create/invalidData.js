...
describe('Invalid data ', () => {
  ...
  it('C22495 - amount = null', async () => {
    const { data } = await banking.depositCreate(socket, '+79215598266', paymentType, currency, null);
    // console.log(data);
    checkErrMsg(data, 400, 'Bad request, amount is required, no default value provided');
  });

  it('C22496 - amount = empty', async () => {
    const { data } = await banking.depositCreate(socket, '79215598276', paymentType, currency, ' ');
    // console.log(data);
    checkErrMsg(data, 400, 'Bad request, amount is invalid');
  });

  it('C22497 - amount = undefined', async () => {
    const { data } = await banking.depositCreate(socket, '79215598206', paymentType, currency, undefined);
    // console.log(data);
    checkErrMsg(data, 400, 'Bad request, amount is required, no default value provided');
  });

  it('C22498 - amount = string', async () => {
    const { data } = await banking.depositCreate(socket, '79215598296', paymentType, currency, 'fjfj');
    // console.log(data);
  ...
  it('C22507 - wallet = number', async () => {
    const { data } = await banking.depositCreate(socket, 111122223330000, paymentType,
      currency, 100);
    // console.log(data);
  ...
  it('C22509 - incorrect paymentType = beeline_rub_test', async () => {
    const { data } = await banking.depositCreate(socket, '+79215598286', 'beeline_rub_test', currency, 1);
    // console.log(data);
