...
describe('Create deposit for btc_usd - RUB', () => {
  ...
  it('C28655 - (+) amount = 1751 & wallet = empty', async () => {
    await banking.depositCreate(socket, '', paymentType, currency, 1751);
 ...
 WHERE id_user = ${user.data.id} ORDER BY id DESC ;`);
  ...
  it('C28656 - min amount & wallet = symbols', async () => {
    await banking.depositCreate(socket, '+79215598289', paymentType, currency, 1750);
    const dbResult = await mysqlConnection.executeQuery(`SELECT * FROM 1win.ma_deposits
 WHERE id_user = ${user.data.id} ORDER BY id DESC ;`);
    // console.log(dbResult);
  ...
  it('C28660 - max amount & wallet = numbers', async () => {
    await banking.depositCreate(socket, '+79215598226', paymentType, currency, 210000);
    const dbResult = await mysqlConnection.executeQuery(`SELECT * FROM 1win.ma_deposits
 WHERE id_user = ${user.data.id} ORDER BY id DESC ;`);
    // console.log(dbResult);
  ...
  it('C28661 - < max amount & wallet = numbers', async () => {
    await banking.depositCreate(socket, '+79215598236', paymentType, currency, 200999);
    const dbResult = await mysqlConnection.executeQuery(`SELECT * FROM 1win.ma_deposits
 WHERE id_user = ${user.data.id} ORDER BY id DESC;`);
    // console.log(dbResult);
...
describe('Create deposite for btc_usd invalid - RUB', () => {
  ...
  it('C28662 - amount double < min amount', async () => {
    const { data } = await banking.depositCreate(socket, '79215598386', paymentType, currency, 1749.6);
    // console.log(data);
    checkErrMsg(data, 400, 'Неверная сумма');
  });

  it('C28663 - amount < min amount', async () => {
    const { data } = await banking.depositCreate(socket, '79215598486', paymentType, currency, 1749);
    // console.log(data);
    checkErrMsg(data, 400, 'Неверная сумма');
  });
