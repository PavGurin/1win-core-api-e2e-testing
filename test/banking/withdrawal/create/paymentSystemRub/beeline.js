...
describe('Withdrawal create with valid test cases ', () => {
  ...
  it('C19328 - (+) With money card_rub + valid wallet', async () => {
    const { data } = await banking.withdrawalCreate(socket, '+79215598286', payment_system, currency, 90.99);
    // console.log
    successWithdrawalCreate(data);
  });

  it(' (-) With money invalid', async () => {
    const { data } = await banking.withdrawalCreate(socket, '+79215598286', payment_system, currency, 10);
    // console.log
    successWithdrawalCreate(data);
  });
  it('(-) With money invali', async () => {
    const { data } = await banking.withdrawalCreate(socket, '+79215598286', payment_system, currency, 10.015);
    // console.log
    successWithdrawalCreate(data);
  });
